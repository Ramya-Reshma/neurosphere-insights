...
import { GraduationCap, FileDown, CheckCircle2 } from 'lucide-react';
...
const stepLabels: Record<Step, string> = { details: 'Student Details', signal: 'Signal Collection', results: 'Results' };
...
export default function StudentModule() {
  ...
  const handleExportPDF = () => {
    ...
    generatePDFReport({
      module: 'Student Focus & Stress Analysis', subjectName: details.studentName,
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-neuro-cyan to-neuro-blue flex items-center justify-center">
        ...
        {stepOrder.map((s, i) => (
          <div key={s} className="flex-1 flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold transition-all ${
              ...
              i === currentIdx ? 'gradient-primary text-primary-foreground glow-primary' :
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'results' && result && eegData && (
          <motion.div key="results" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-5">
            ...
            <ResultsDisplay result={result} recommendations={recommendations} moduleType="student" eegData={eegData} />
