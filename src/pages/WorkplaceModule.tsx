...
import { Building2, FileDown, CheckCircle2 } from 'lucide-react';
...
const stepLabels: Record<Step, string> = { details: 'Employee Details', signal: 'Signal Collection', results: 'Results' };
...
export default function WorkplaceModule() {
  ...
  const handleEEGData = async (data: EEGData) => {
    ...
    setResult(res);
    const recs = getRecommendations(res, 'workplace');
    setRecommendations(recs);
    setStep('results');
    try {
      await saveAnalysis({
        moduleType: 'workplace', subjectName: details.name,
        subjectDetails: { Name: details.name, Age: details.age, Department: details.department, Role: details.role, 'Work Hours': details.workHours, Reason: details.reason },
  ...
  const handleExportPDF = () => {
    ...
    generatePDFReport({
      module: 'Workplace Cognitive Stress Monitoring', subjectName: details.name,
  ...
  return (
    <div className="p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-neuro-blue to-neuro-purple flex items-center justify-center">
      ...
      <AnimatePresence mode="wait">
        ...
        {step === 'results' && result && eegData && (
          <motion.div key="results" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="space-y-5">
            ...
            <ResultsDisplay result={result} recommendations={recommendations} moduleType="workplace" eegData={eegData} />
